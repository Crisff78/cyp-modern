...
```
{
 ...
 charges: [{id,clientId,service,concept?:string,currency?:string,note?:string,amount:number,collected:number,dueDate:string,required:boolean,status:'pending'|'partial'|'paid'|'cancelled'}],
