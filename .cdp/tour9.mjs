...
const ocultarModulos = async () => run(`(() => { for (const w of [...document.querySelectorAll('.x-window')]) { const t=(w.querySelector('.x-window-header-text')?.innerText||'').trim(); if (t && t!=='Opciones' && t!=='Monitor de Cobradores' && getComputedStyle(w).display!=='none') w.style.display='none'; } return 1; })()`);
const visibles = async () => run(`(() => [...document.querySelectorAll('.x-window')].filter(w=>getComputedStyle(w).display!=='none').map(w=>w.id))()`);
...
const desplegar = async (grupo) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (p !== g.cab) { await limpiar(); continue; }
...
for (const [nombre, grupo, id] of ITEMS) {
  ...
  let cap = null;
  if (nuevos.length) cap = await capturaUna(nuevos[0]);
  if (!cap) {
    const top = await run(`(() => { const o=[...document.querySelectorAll('.x-window')].filter(w=>getComputedStyle(w).display!=='none').map(w=>({w,z:parseInt(w.style.zIndex||'0',10)})).filter(o=>{const t=(o.w.querySelector('.x-window-header-text')?.innerText||'').trim();return t&&t!=='Opciones'&&t!=='Monitor de Cobradores'&&t!=='Confirm';}).sort((a,b)=>b.z-a.z)[0]; return o?o.w.id:null; })()`);
    if (top) cap = await capturaUna(top);
  }
  const evs = c.drain();
  ...
  for (const r of reqs) if (fin.has(r.id)) {
    ...
    try { const rb = await c.send('Network.getResponseBody', { requestId: r.id }); if (rb && rb.body) r.respBody = rb.body.slice(0, 600); } catch (e) {}
  }
  if (cap) cap.red = reqs.slice(0, 20);
  result[nombre] = cap || { err: 'no-window', expandio };
}
fs.writeFileSync('.cdp/tour9.json', JSON.stringify(result, null, 1));
console.log('fin tour9 con', Object.keys(result).length, 'modulos');
c.close();
