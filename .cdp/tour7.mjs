...
const enPunto = async (x, y) => run(`(() => { const el=document.elementFromPoint(${x},${y}); if(!el) return null; let s = el.id || el.tagName; let p = el; for(let i=0;i<4 && p; i++){ if(p.id){s=p.id; break;} p=p.parentElement; } return s; })()`);
...
const ocultarVentanas = async () => run(`(() => {
  const hidden = [];
...
for (const [nombre, grupo, id] of ITEMS) {
  ...
  if (cap) { cap.red = reqs.slice(0, 20); cap.stale = (cap.title === antes); } else { cap2 = null; }
  result[nombre] = cap || { err: 'no-window' };
  await restaurar(hidden);
}
fs.writeFileSync('.cdp/tour7.json', JSON.stringify(result, null, 1));
console.log('fin tour7 con', Object.keys(result).length, 'modulos');
c.close();
