...
const dismissConfirm = async () => {
  ...
  if (no) { await clickAt(no.x, no.y); await sleep(1000); return true; }
  return false;
};
// cerrar ventanas de modulo por destroy sin confirm
const closeModuleWindows = async () => {
  ...
  for (let i=0;i<6;i++) {
    const res = await run(`(() => {
      ...
      if (!cmp) { w.style.display='none'; return { hidden:w.id }; }
      cmp.destroy();
      return { destroyed: w.id };
    })()`);
    if (!res) break;
    await sleep(700);
    await dismissConfirm();
...
await closeModuleWindows();

const capture = async () => {
  await dismissConfirm();
  return await run(`(() => {
    const keep = new Set(['Opciones','Confirm']);
...
await closeModuleWindows();
fs.writeFileSync('.cdp/tour3.json', JSON.stringify(result, null, 1));
console.log('fin tour3 con', Object.keys(result).length, 'modulos');
cdp.close();