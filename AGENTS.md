...
**Last updated:** 2026-09-25 por Codex — La suite de nueve reportes de Cargos/Cobros ya usa panel MDI compacto, filtros específicos, grilla con fila totalizadora y acciones compartidas de refrescar, imprimir limpio y exportar mediante Blob. Los ocho reportes adicionales muestran títulos y columnas alineados al legacy desde el snapshot local. Resumido General agrega cobros, depósitos, entregas y pagos por cobrador con diferencias entre cobrado/depositado y entregado/pagado; Cobros por Servicios lista los cobros por cliente, servicio y concepto. Las opciones 0/1/7 descargan TXT/CSV delimitado, XML, HTML y RTF se generan localmente, y PDF queda como TXT tabular simulado. Los otros siete reportes fuera del alcance de esta pasada conservan su vista preliminar. El reporte **Cargos pendientes de cobros de los clientes** filtra cargos reales del snapshot por fecha/moneda y calcula importe, recibido y pendiente. **Cargos Recurrentes** usa la grilla MDI con filtros, datos de importe/activo/fecha de registro, totales y formulario validado; el importe se bloquea al usar el importe del concepto. El dispatcher de ventanas ahora renderiza directamente `RecurringChargesOperationalView` y ya no incluye esta página en la ruta genérica `LegacyOperationView`. Su cabecera, filtros, controles de entrada y cabeceras de grilla ahora comparten el estilo compacto azul/gris de Cobros. Los monitores C/Z/R muestran CobranzaMapas con MapViewer en un iframe MDI; arma rutas `/maps/cobrador`, `/maps/zona` o `/maps/ruta`, envía `SET_SESSION` al origin exacto de `VITE_MAPS_ORIGIN` y usa UUID mock validado hasta recibir una sesión compatible. Cobros incluye cancelación MDI en dos pasos con nota y baja lógica en mock, selector de alcance para imprimir, vista previa térmica imprimible (también desde `Guardar e Imp.`) y selector de alcance para mapa con estilos Hybrid/Roadmap/Satellite/Terrain y marcadores de clientes geolocalizados. La baja lógica simulada guarda la nota y excluye movimientos cancelados de balances mock; el servidor no expone aún una ruta de cancelación de cobros. Se conserva el recibo en memoria sin API de guardado. **Depósitos por Cobradores** usa ahora toolbar legacy y layout MDI de tres paneles con filtros, selección, detalles de denominaciones, alta con desglose, impresión, aceptación y cancelación con motivo. La grilla de denominaciones del modal quedó en solo lectura y vacía hasta pulsar Refrescar; únicamente Fecha, Moneda, Cobrador y Nota son editables. Refrescar carga efectivo disponible en DOP y muestra ceros para monedas sin desglose disponible en el snapshot; los importes se muestran con la moneda seleccionada. La API actual permite crear, aceptar y cancelar depósitos, pero no editar movimientos ni persistir fecha/moneda/nota del formulario o motivo de cancelación; esos campos adicionales de edición/formulario permanecen locales en el frontend. **Descargos** ahora abre una vista MDI compacta con toolbar, filtros, grilla seleccionable y totales; alta e inactivación usan la API existente. No hay endpoint de edición para descargos, por lo que editar y los campos Fecha/Moneda/Servicio/Nota se mantienen locales en la sesión de UI. `npm run check`: TypeScript correcto, 21 tests pass, 1 skip esperado por PostgreSQL no configurado y builds de los tres workspaces correctos; Vite mantiene un aviso de bundle superior a 500 kB.
...
**Update 2026-09-24 — Cuadres Diarios MDI:** Se reemplazó la plantilla genérica por toolbar legacy, filtro lateral y grilla compacta con desglose anidado de Total/Efectivo/Cheque, balance por fórmula, selección y reordenamiento local. Generar por fecha, editar Balance del Día con desglose de denominaciones y cerrar/reversar con validación del día anterior operan en estado local; no se modificó API ni backend. `npm run check`: typechecks y builds correctos, 21 pruebas aprobadas y 1 omitida por PostgreSQL de integración no configurado; Vite conserva el aviso de bundle de Admin superior a 500 kB.
